...
class _AdminResearchersPageState extends State<AdminResearchersPage> {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setDialogState) {
          ...
          return AlertDialog(
            title: const Text('Add Field Researcher'),
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 10),
                    TextField(controller: specCtrl, decoration: const InputDecoration(labelText: 'Research Specialization')),
                    const SizedBox(height: 10),
                    TextField(controller: passCtrl, decoration: const InputDecoration(labelText: 'Initial Password *', helperText: 'Researcher will use this to sign in')),
                    const SizedBox(height: 14),
                    const Text('Assign School Institutions:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  try {
                    await context.read<AdminDataProvider>().addResearcher(
                    ...
                    if (mounted) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Researcher created successfully!'), backgroundColor: AppTheme.successColor),
                      );
                    }
                  } catch (e) {
                    if (mounted) {
                ...
                child: const Text('Create Researcher'),
              ),
            ],
          );
        },
  ...
  void _showEditResearcherDialog(UserModel researcher) {
    final nameCtrl = TextEditingController(text: researcher.name);
    final phoneCtrl = TextEditingController(text: researcher.phone ?? '');
    final specCtrl = TextEditingController(text: researcher.specialization ?? '');
    final selectedSchoolIds = List<String>.from(researcher.schoolIds);
    String selectedRole = researcher.role.toDbString();
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setDialogState) {
          ...
          return AlertDialog(
            title: const Text('Edit Researcher Profile & Role'),
            content: SingleChildScrollView(
              child: ConstrainedBox(
                constraints: const BoxConstraints(maxWidth: 480),
                child: Column(
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await context.read<AdminDataProvider>().updateResearcher(
                    researcher.id,
                    name: nameCtrl.text.trim(),
                    phone: phoneCtrl.text.trim(),
                    specialization: specCtrl.text.trim(),
                  ...
                  if (mounted) {
                    Navigator.pop(ctx);
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Researcher updated!'), backgroundColor: AppTheme.successColor),
                    );
                  }
                },
  ...
  void _showDeleteDialog(UserModel researcher) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Delete Researcher Account?'),
        content: Text('Are you sure you want to remove ${researcher.name} (${researcher.email})?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              if (mounted) {
                Navigator.pop(ctx);
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Researcher deleted.')));
              }
            },
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = researchers.where((r) {
      ...
      final matchesQ = r.name.toLowerCase().contains(q) ||
          r.email.toLowerCase().contains(q) ||
          (r.specialization?.toLowerCase().contains(q) ?? false);
      if (!matchesQ) return false;
      if (_schoolFilter != 'all' && !r.schoolIds.contains(_schoolFilter)) return false;
      return true;
    }).toList();
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: TextField(
                    decoration: const InputDecoration(
                      hintText: 'Search by researcher name, email, or specialization...',
                ...
                const SizedBox(width: 12),
                ElevatedButton.icon(
                  onPressed: _showAddResearcherDialog,
            ...
            Expanded(
              child: filtered.isEmpty
                  ? const Center(
                      child: Text('No researchers found matching query.', style: TextStyle(color: Colors.grey)),
                    )
                  : ListView.separated(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text('${r.email} • ${r.phone ?? "No phone"} • ${r.specialization ?? "General Researcher"}', style: TextStyle(fontSize: 12, color: Theme.of(context).textTheme.bodySmall?.color)),
                                    const SizedBox(height: 4),
                                    Text(
                                      '${r.schoolIds.length} assigned school(s)',
                              ...
                              IconButton(
                                ...
                                tooltip: 'Edit',
                                onPressed: () => _showEditResearcherDialog(r),
                              ),
                              IconButton(
